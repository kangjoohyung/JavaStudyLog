...
public class TransactionDAOmeError {
  /**
   * 계좌이체 기능 구현
   * */
	public void transfer(String inputAccount, String outputAccount, int money) {
		...
		try {
			...
			//4. 조회한 금액이 1000 미만이면 commit 한다. 그 외는 모두 rollback한다.
			con.commit();
	...
	public int withDraw(Connection con, String outputAccount, int money)throws SQLException {//출금계좌번호, 금액
		...
		try {
			...
			ps.setInt(money, result);
			ps.setString(2, outputAccount);
			result=ps.executeUpdate();
		}finally { //닫기-connection은 다른기능때문에 유지
	 ...
	 * 입금하기
	 */
	public int deposit(Connection con, String inputAccount, int money) throws SQLException { //입금 계좌번호, 금액
		PreparedStatement ps=null;
		String sql="update bank set balance=balance+? where account=?";
		int result=0;
		try {
	...
	/**
	 * 잔액조회하기
	 * @return : true이면 잔액초과, false이면 초과아니다 (1000)
	 */
	...
	public static void main(String[] args) {
		TransactionDAOmeError dao = new TransactionDAOmeError();
		System.out.println("--1. 출금계좌 오류----");
		dao.transfer("A02", "A05",200);
		//입금, 출금, 금액
		
		System.out.println("--2. 입금계좌 오류----");
		dao.transfer("A04", "A01",200);
		//입금, 출금, 금액
		
		
		System.out.println("--3. 입금계좌의 총액 1000원 이상인경우----");
		dao.transfer("A02", "A01",700);
		//입금, 출금, 금액
		
		System.out.println("--4. 성공----------");
		dao.transfer("A02", "A01",100);
		//입금, 출금, 금액
	}
}
